...
export const getclientesxid = async (req, res) => {
  try {
    const result = await conmyslq.query(
      'SELECT * FROM _clientes WHERE cli_id = $1', [req.params.id]);
...
export const getclientesxcedula = async (req, res) => {
  try {
    const result = await conmyslq.query(
      'SELECT * FROM _clientes WHERE cli_identificacion = $1', [req.params.cedula]);
...
export const postInsertarcliente = async (req, res) => {
  try {
    ...
    const result = await conmyslq.query(
      'INSERT INTO _clientes(cli_identificacion, cli_nombre, cli_telefono, cli_correo, cli_direccion, cli_pais, cli_ciudad) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING cli_id',
...
export const putcliente = async (req, res) => {
  try {
    ...
    await conmyslq.query(
      'UPDATE _clientes SET cli_identificacion=$1, cli_nombre=$2, cli_telefono=$3, cli_correo=$4, cli_direccion=$5, cli_pais=$6, cli_ciudad=$7 WHERE cli_id=$8',
...
export const deletecliente = async (req, res) => {
  try {
    const result = await conmyslq.query(
      'DELETE FROM _clientes WHERE cli_id = $1', [req.params.id]);
